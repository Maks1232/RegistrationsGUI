...
def position_update(WIN, app_logo):
    ...
    _level_drop_down = pygame.Rect(
        0.9 * WIN.get_width() - _drop_down[2],
        20 + 1.1 * app_logo.get_height(),
        0.25 * WIN.get_width(),
        0.04 * WIN.get_height(),
    )

    _play_button = pygame.Rect(
        0.65 * WIN.get_width(),
        0.82 * WIN.get_height(),
        0.25 * WIN.get_height(),
        0.04 * WIN.get_height(),
    ...
    _mode_button = pygame.Rect(
        WIN.get_width() / 2 - 0.0625 * WIN.get_width(),
        20 + 1.1 * app_logo.get_height(),
        0.125 * WIN.get_width(),
        0.04 * WIN.get_height(),
    )
...
def draw_rect(position, text, button_color, WIN):
    ...
    write_text(
        text.value if isinstance(text, Enum) else text,
...
def draw_list(position, options, active_list, active_choice, WIN):
    draw_rect(
        position,
        active_choice,
        Color.ACTIVE_GREEN.value if active_list else Color.GREEN.value,
        WIN,
    )
    ...
    if active_list:
        for i, option in enumerate(options):
            ...
            write_text(
                option.value if isinstance(option, Enum) else option,
                (position[0] + position.width // 2, y + position.height // 2),
                WIN,
            )
...
def nick_input(WIN):
    ...
    _text_input = pygame_gui.elements.UITextEntryLine(
        relative_rect=pygame.Rect(
            (WIN.get_width() / 2 - _frame[0] / 2, WIN.get_height() / 2 - _frame[1] / 2),
            _frame,
        ),
        manager=_manager,
...
def get_user_name(clock, WIN):
    ...
    font = pygame.font.Font(None, 32)
    text = font.render(
        "Wprowadź nazwę użytkownika i wciśnij klawisz ENTER:",
        True,
    ...
    while _run:
        ...
        for _event in pygame.event.get():
            if _event.type == pygame.VIDEORESIZE:
                ...
                _event.type == pygame_gui.UI_TEXT_ENTRY_FINISHED
                and _event.ui_object_id == "#main_text_entry"
            ):
                _run = False
                return _event.text
...
def handle_drop_down_event(
    _drop_down, _options, _full_list_flag, _active_option, event
):
    if _full_list_flag:
        for _i, _option in enumerate(_options):
            _y = _drop_down.y + (_i + 1) * _drop_down.height
            square_option = pygame.Rect(
                _drop_down.x, _y, _drop_down.width, _drop_down.height
            )
    ...
    return (
        _active_option.value if isinstance(_active_option, Enum) else _active_option
    ), _full_list_flag
...
def main_function():
    ...
    while run:
        ...
        drop_down, level_drop_down, play_button, exit_button, mode_button = (
            position_update(WIN, app_logo)
        )
        WIN.fill(Color.WHITE.value)
        WIN.blit(
            app_logo,
            (
                0.5 * WIN.get_width() - 0.5 * app_logo.get_width(),
                0.02 * WIN.get_height(),
            ),
        )
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                if drop_down.collidepoint(event.pos):
                    ...
                    active_option, full_list = handle_drop_down_event(
                        drop_down, voivodeship_options, full_list, active_option, event
                    )
                elif full_level_list:
                    active_level_option, full_level_list = handle_drop_down_event(
                        level_drop_down,
                        Level,
                        full_level_list,
                        active_level_option,
                ...
                if play_button.collidepoint(event.pos):
                    if (
                        not active_option == "Wybierz województwo"
                        and not active_level_option == "Wybierz poziom"
                    ):
                        if (
                            active_option == voivodeship_options[-1]
                            and active_level_option == Level.EASY.value
                        ):
                            Game.config_notification()
                        else:
                            game_instance = Game(
                                WIN, active_option, active_level_option, mode, nickname
                            )

        draw_rect(
            mode_button,
            tribes[mode],
            Color.DARK_GREY.value if mode != 0 else Color.GREY.value,
            WIN,
        )
        draw_list(
            drop_down,
            ...
            active_choice=active_option,
            WIN=WIN,
        )
        draw_list(
            level_drop_down,
            Level,
            active_list=full_level_list,
            active_choice=active_level_option,
            WIN=WIN,
        )
        ...
        draw_rect(
            ...
            (
                Color.GREEN.value
                if active_option != "Wybierz województwo"
                and active_level_option != "Wybierz poziom"
                else Color.GREY.value
            ),
            WIN,
        )
